...
output_path = os.path.login(login_user, clean_csv_output_path)

class DataCleaning:
    def __int__(self, input_path, output_path):
        self.input_path = input_path
        self.output_path = output_path

        def load_dataset(self):
            df = pd.DataFrame(self.input_path)
            return df
        ...
        def handel_missing_value(self, df):
            ...
            for col in feature_cols:
                df[col] = df[col].fillna(df[col].meadian())
